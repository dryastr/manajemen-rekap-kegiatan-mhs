...
class RekapAttandanceMHSParentController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (!$parentMhs || !$parentMhs->student) {
            return redirect()->back()->with('error', 'Data anak Anda tidak ditemukan atau tidak lengkap.');
        }

        $studentId = $parentMhs->student->id;
        ...
        $student = $parentMhs->student->load('user');

        return view('parent.rekap_attendance_mhs.index', compact('attendances', 'coursesAttended', 'selectedCourseId', 'formattedRecap', 'student'));
    }
}
